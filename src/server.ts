...
async function sendContactEmail(request: Request, env: ContactEnv): Promise<Response> {
  ...
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      if (url.pathname === "/api/contact") {
        ...
        try {
          return await sendContactEmail(request, env as ContactEnv);
        } catch (error) {
          console.error(error);
          return jsonResponse(
            { error: "We could not send your message. Please call (801) 917-4049." },
            { status: 500 },
          );
        }
      }
